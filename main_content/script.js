...
function updateTableHeaders() {
  ...
  // Check if SN column is already added to avoid duplicates
...
function renderTable(filtered) {
  ...
  if (!filtered || filtered.length === 0) {
    ...
    row.innerHTML = `<td colspan="6">No results found</td>`; // Updated colspan to match columns
...
// Restore previous filters & results
...
// Utility function for description line formatting
function formatDescription(desc) {
  if (!desc) return "NA";
  return desc
    .replace(/(\d+\s*[).])/g, '<br>$1') // <br> before pointers like "1)" or "2)"
    .replace(/<br>1[).]/, '1)'); // Remove extra first <br>
}
